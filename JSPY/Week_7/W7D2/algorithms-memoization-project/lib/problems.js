...
function minChange(coins, amount, memo = {}) { // tabulated
    if (amount < Math.min(...coins)) return 0;
    for (let i=1; i <= amount; i++){
        if (coins.includes(i)){
            memo[i] = 1;
        } else {
            for (coin of coins){
                if (!memo[i] || memo[i-coin] + 1 < memo[i]) memo[i] = memo[i-coin] + 1;
            };
        };
    };
    return memo[amount];
...
function minChange(coins, amount, memo = {}) { //memoized
