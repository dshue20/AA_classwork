...
class MinMaxStack {
    ...
    push(val) {
        ...
        } else if (newNode.value >= this.maxArr[this.maxArr.length-1].value){
            this.maxArr.push(newNode);
        } else if (newNode.value <= this.minArr[this.minArr.length-1].value){
            this.minArr.push(newNode);
        };
        if (!this.top) {
            this.top = newNode;
    ...
    pop() {
        if (this.top === this.maxArr[this.maxArr.length-1]){
            this.maxArr.pop();
        };
        if (this.top === this.minArr[this.minArr.length-1]){
            this.minArr.pop();
        };
        if (!this.top) {
            return null;
    ...
    max(){
        return this.maxArr.length ? this.maxArr[this.maxArr.length-1] : null;
    }

    min(){
        return this.minArr.length ? this.minArr[this.minArr.length-1] : null;
    }
}
