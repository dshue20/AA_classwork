...
function balancedParens(str) {
    ...
    for (let i=0; i < str.length; i++){
        if (str[i] === '('){
            ...
            if (!parens || prev[prev.length-1] === '[' || prev[prev.length-1] === '{') return false;
            parens--;
            prev.pop();
        } else if (str[i] === ']'){
            if (!brackets || prev[prev.length-1] === '(' || prev[prev.length-1] === '{') return false;
            brackets--;
            prev.pop();
        } else if (str[i] === '}'){
            if (!squigglys || prev[prev.length-1] === '[' || prev[prev.length-1] === '(') return false;
            squigglys--;
            prev.pop();
        };
    };
