...
//         if (i < nums.length) result = stepper(nums.slice(i), memo)
//         memo[nums.length] = result;
...
function maxNonAdjacentSum(nums, memo={}){
    ...
    memo[nums.length] = Math.max(...[nums[0] + maxNonAdjacentSum(nums.slice(2), memo), nums[0] + maxNonAdjacentSum(nums.slice(3), memo), nums[1] + maxNonAdjacentSum(nums.slice(3), memo), nums[1] + maxNonAdjacentSum(nums.slice(4), memo)])
...
function minChange(coins, amount) {
    ...
    for (let i=1; i <= amount; i++){
        if (coins.includes(i)){
            ...
            let prevSums = [];
            coins.forEach(coin => {
                if (i - coin > 0) prevSums.push(table[i-coin])
            })
            table[i] = Math.min(...prevSums) + 1;
        }
    }
    return table[amount];
}
